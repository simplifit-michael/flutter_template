...
import 'error_handling/exceptions/data_layer_exception.dart';
import 'error_handling/export.dart';
import 'model/base_dto.dart';
...
abstract class BaseRepository {
  ...
  Future<Either<FailureType, TDomain>> //
      get<TModel extends BaseDto<TDomain>, TDomain>({
  ...
  Future<Either<FailureType, List<TDomain>>> //
      getList<TModel extends BaseDto<TDomain>, TDomain>({
    ...
    try {
      ...
      return Right(model.map((e) => e.toModel()).toList());
    } on Exception catch (e) {
      final bool ignoreError = //
          e is ApiException && expectedErrors.contains(e.code);
      if (!ignoreError) {
